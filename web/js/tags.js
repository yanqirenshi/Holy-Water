...
riot.tag2('impure-card-large_tab_deamon', '<div> <div style="padding-left:22px; padding-right:22px;"> <div class="selected" style="padding: 11px 22px;"> <p>{selectedDeamon()}</p> </div> <div style="margin-top:8px; display:flex; justify-content:space-between;"> <button class="button is-small" onclick="{clickRevert}" disabled="{isDisabled()}">Revert</button> <button class="button is-small is-danger" onclick="{clickSave}" disabled="{isDisabled()}">Save</button> </div> </div> <div class="selector"> <h1 class="title is-5">Select Deamon:</h1> <div> <input class="input is-small" type="text" placeholder="Filter" onkeyup="{keyup}"> <div class="deamons"> <button each="{obj in deamons()}" class="button is-small" deamon-id="{obj.id}" onclick="{selectDeamon}"> {obj.name_short} : {obj.name} </button> </div> </div> </div> </div>', 'impure-card-large_tab_deamon > div { display:flex; flex-direction: column; height: 100%; } impure-card-large_tab_deamon .selected { background: #efefef; padding: 22px; border-radius: 3px; } impure-card-large_tab_deamon .selector { flex-grow: 1; margin-top: 22px; } impure-card-large_tab_deamon .selector > div { padding: 0px 22px; display: flex; flex-direction: column; } impure-card-large_tab_deamon .selector .title { margin-bottom: 5px; } impure-card-large_tab_deamon .selector .input { margin-bottom: 11px; } impure-card-large_tab_deamon .selector .deamons { flex-grow: 1; display: flex; flex-wrap: wrap; } impure-card-large_tab_deamon .selector .deamons > * { margin-right: 11px; margin-bottom: 11px; }', '', function(opts) {
     this.filter = null;
...
riot.tag2('impure_page-controller', '<div class="controller-container"> <div class="operators"> <button class="button is-small" onclick="{clickRefresh}">Refresh</button> <span style="flex-grow:1;"></span> <button class="button is-small" disabled>作業開始</button> <button class="button is-small" disabled>作業終了</button> <button class="button is-small" disabled>呪文詠唱</button> <button class="button is-small" disabled>後続作成</button> <button class="button is-small" disabled>埋葬</button> </div> </div>', 'impure_page-controller > .controller-container { background: #FEF264; padding: 11px 22px; border-radius: 3px; } impure_page-controller .operators { display:flex; } impure_page-controller .operators > * { margin-right:11px; } impure_page-controller .operators > *:last-child { margin-right:0px; }', '', function(opts) {
     this.clickRefresh = () => {
         this.opts.callback('refresh');
     };
});
...
riot.tag2('impure_page_tab-chains', '<section class="section" style="padding:0px;"> <div class="container"> <h1 class="title hw-text-white">準備中</h1> </div> </section>', '', '', function(opts) {
});

riot.tag2('impure_page_tab-incantation', '<section class="section" style="padding:0px;"> <div class="container"> <h1 class="title hw-text-white"></h1> <div class="contents"> <table class="table is-bordered is-striped is-narrow is-hoverable"> <thead> <tr> <th rowspan="2">ID</th> <th rowspan="2" colspan="2">Date</th> <th colspan="2">祓魔師</th> <th rowspan="2">Spell</th> </tr> <tr> <th>ID</th> <th>Name</th> </tr> </thead> <tbody> <tr each="{rec in opts.source}"> <td>{rec.id}</td> <td>{dt(rec.incantation_at)}</td> <td>{week(rec.incantation_at)}</td> <td>{rec.angel_id}</td> <td>{rec.angel_name}</td> <td> <description-markdown source="{contents(rec.spell)}"></description-markdown> </td> </tbody> </table> </div> </div> </section>', 'impure_page_tab-incantation description-markdown pre { padding:0px; } impure_page_tab-incantation > .section { margin-bottom: 222px;}', '', function(opts) {
